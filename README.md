...
**Dots** (`.carousel-dots`, same file) are the mobile affordance — the arrows are hidden below 768px because a 44px circle over a driver's photo was worse than the swipe it replaced, which leaves the dots as the only always-visible sign that the row is browsable at all on a phone. A dot represents a **page** (one viewport-width of cards), not a card: on a phone one card fills the track, so a page and a card are the same thing and it reads as "one dot per driver"; on desktop several cards share a page, so the row stays a handful of dots as more drivers are added instead of growing one-for-one with the roster. Don't switch this back to one-dot-per-card — the target for card *N* is `N × cardWidth`, which overshoots the track's real max scroll the moment more than one card is visible at a time, the browser silently clamps it, and two dots end up pointing at the same position while the "active" one desyncs from what's on screen. That was the exact bug this shipped with initially.

**Mobile-only trap:** the dots are the last thing in the section, and the fixed `.mobile-book-bar` sits at `z-index: 1200` across the bottom ~74px of every phone screen, swallowing taps meant for whatever is underneath it — the same clearance `body { padding-bottom: 74px }` exists to protect at the true end of the page. Without matching clearance here, the dots land in that dead strip the instant a visitor scrolls to finish reading a card, and taps intended for a dot hit the bar instead. That's what `.carousel-dots { margin-bottom: 74px }` inside the mobile breakpoint is for — if a future row of cards ends in something tappable near the bottom of its section, it needs the same margin, and any check of "does this work on a phone" needs to test at the scroll position a reader actually stops at (where the element in question is near the bottom of the viewport), not a `scrollIntoView({block:'center'})` sanity check — that hides exactly this class of bug because it never rests the element against the bottom edge.
